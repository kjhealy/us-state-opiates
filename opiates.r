...
data <- read.csv("data/opiate-deaths-1999-2014.csv")
...
## of "data"
data.lab <- data %>% filter(Year == 2014)

## Then increment the year slightly to create a gap for the labels
data.lab$Year <-  data.lab$Year

pdf(file = "figures/opiate-deaths-state-level-lineplots-by-region-facet.pdf",
    height = 6, width = 13)
...
p1 <- p0 + geom_line(alpha = 0.7, lty = 3) +
    ...
    ggtitle("State-Level Opiate Death Rates by Census Region, 1999-2014\n")

print(p1)
credit()
dev.off()

png(file="figures/opiate-deaths-us-ts-region.png", height=1600, width=3200,
